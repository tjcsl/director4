...
class RootShellSession(asyncssh.SSHServerSession):  # type: ignore
    ...
    def connection_lost(self, exc: Optional[Exception]) -> None:
        logger.info("[fake root shell] %s disconnected", self.remote_addr)

    def exec_requested(self, command: str) -> bool:
        logger.info("[fake root shell] %s Requested command: %r", self.remote_addr, command)
    ...
    def pty_requested(
        ...
        logger.info(
            "[fake root shell] %s Requested PTY: %r %r %r", self.remote_addr, term_type, term_size, term_modes
        )
        return True
    ...
    def data_received(self, data: Union[str, bytes], datatype: Any) -> None:
        ...
        if self.request_type == "shell":
            if "\n" in self.buf:
                ...
                logger.info("[fake root shell] %s Typed command: %r", self.remote_addr, command)
                if command:
    ...
    def signal_received(self, signal: Any) -> None:
        logger.info("[fake root shell] %s Signal received: %r", self.remote_addr, signal)
    ...
    def terminal_size_changed(
        ...
        logger.info(
            "[fake root shell] %s Terminal size changed: %r", self.remote_addr, (width, height, pixwidth, pixheight)
        )
