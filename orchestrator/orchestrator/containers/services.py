...
def create_service(client: DockerClient, service_name: str) -> Optional[Service]:
    ...
    # Mapping ports
    endpoint_spec = EndpointSpec(mode="vip", ports={8080: 8080})

    image_name = "site_test_1"
    service = client.services.create(image=image_name, name=service_name, restart_policy=restart_policy, resources=resource, endpoint_spec=endpoint_spec)
