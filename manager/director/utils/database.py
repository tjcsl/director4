...
def create_database(database: Database) -> None:
    if database.host.dbms == "postgres":
        with open_cursor(database.host, dbname="postgres") as cursor:
            cursor.execute(
                "SELECT 1 FROM pg_catalog.pg_user WHERE usename = %s", (database.username,)
            )
            if cursor.rowcount == 0:
                cursor.execute(
                    psql.SQL("CREATE USER {} WITH PASSWORD %s").format(
                        psql.Identifier(database.username)
                    ),
                    (database.password,),
                )
            else:
            ...
            if cursor.rowcount == 0:
                cursor.execute(
                    psql.SQL("CREATE DATABASE {} WITH OWNER = %s").format(
                        psql.Identifier(database.db_name)
                    ),
                    (database.host.admin_username,),
                )
            cursor.execute(
                psql.SQL("GRANT ALL PRIVILEGES ON DATABASE {} TO {}").format(
                    psql.Identifier(database.db_name), psql.Identifier(database.username)
                )
            )
        ...
        with open_cursor(database.host, dbname="mysql") as cursor:
            cursor.execute(
                ...
                (mysql_clean_identifier(database.username),),
            )
            if cursor.rowcount == 0:
                cursor.execute(
            ...
            cursor.execute(
                "CREATE DATABASE IF NOT EXISTS {}".format(mysql_clean_identifier(database.db_name))
            )
            cursor.execute(
                "GRANT ALL ON {} . * TO {};".format(
                    mysql_clean_identifier(database.db_name),
                    mysql_clean_identifier(database.username),
                )
            )
...
def delete_database(database: Database) -> None:
    if database.host.dbms == "postgres":
        with open_cursor(database.host, dbname="postgres") as cursor:
            cursor.execute(
                psql.SQL("DROP DATABASE IF EXISTS {}").format(psql.Identifier(database.db_name))
            )
            ...
            cursor.execute(
                psql.SQL("DROP USER IF EXISTS {}").format(psql.Identifier(database.username))
            )
    elif database.host.dbms == "mysql":
        with open_cursor(database.host, dbname="mysql") as cursor:
            cursor.execute(
                "DROP DATABASE IF EXISTS {};".format(mysql_clean_identifier(database.db_name))
            )

            cursor.execute(
                "DROP USER IF EXISTS {}@'%%';".format(mysql_clean_identifier(database.username))
            )
    else:
        raise ValueError("Unknown DBMS {!r}".format(database.host.dbms))
